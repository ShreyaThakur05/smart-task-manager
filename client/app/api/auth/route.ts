...
export async function POST(request: NextRequest) {
  try {
    ...
    if (email === 'shreya_list@demo.com' && password === 'test123') {
      const token = jwt.sign({ userId: 'demo-user', email }, JWT_SECRET, { expiresIn: '7d' })
    ...
    if (action === 'register') {
      ...
      const token = jwt.sign({ userId: result.insertedId.toString(), email: email.toLowerCase() }, JWT_SECRET, { expiresIn: '7d' })
